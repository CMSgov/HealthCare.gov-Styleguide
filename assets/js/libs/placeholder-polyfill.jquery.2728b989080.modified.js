...
(function($) {
    ...
    function positionPlaceholder(placeholder,input){
        ...
        placeholder.css({
            // When computing top position, do not add padding-top since in bootstrap 3
            // all elements use box-sizing: border-box
            top: position.top,
            left: position.left + parseInt(input.css('padding-left'), 10),
    ...
    $.fn.placeHolder = function(config) {
        ...
        return $(this).each(function(index) {
            ...
            label = input.closest('label, .form-group');
            // Modified by liyanchang 09/04/2014
            // Change: Comment out the removal of the placeholder
            // Rationale: Turns out that if you have a hidden input field,
            // it'll render the css positioning wrong (since you're not
            // positioned yet). No problem - just rerender. But as it turns
            // out the original data has been removed, ie the placeholder
            // text. Therefore, the easy fix is to not remove it.
            // A more complicated move might be to re-generate the css
            // positioning.
            // Possible consequnces: Not sure. It might be that a browser
            // renders both the polyfill and the placeholder? My understanding
            // is that this favors native placeholder support (unless you use
            // force rendering).
            ...
            placeholder = $('<span>').addClass(o.options.className).text(text).insertAfter(input);
